...
import (
	"crypto/rand"
	"errors"
	"github.com/gwsee/crypto-gm/internal/sm2"
	"io"
)
...
//BatchVerify verify a batch of signature
func (sv *SM2) BatchVerify(publicKey, signature, digest [][]byte) (valid bool, err error) {
	err = sm2.BatchVerify(publicKey, signature, digest)
	if err == nil {
		return true, nil
	}
	return false, err
