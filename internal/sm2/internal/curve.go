...
//ComplementCoordinates_32bit
//the result is placed in 'x'
//tildeY is Y rightmost bit
func ComplementCoordinates_32bit(x *big.Int, tildeY byte) error {
	//part1 appendix B.1.4, p = 3 mod 4
	//4u+3=p, u= 3fffffffbfffffffffffffffffffffffffffffffc00000003fffffffffffffff
	var u = GetInt().SetBits([]big.Word{0x4000000000000000, 0xffffffffc0000000, 0xffffffffffffffff, 0x3fffffffbfffffff})
	var x3 = GetInt()
	x3.Mul(x, x)
	x3.Mul(x3, x)
	x3.Sub(x3, x)
	x3.Sub(x3, x)
	x3.Sub(x3, x)     //x^3-3x
	x3.Add(x3, sm2.B) //x^3-3x+b
	x3.Mod(x3, sm2.P)
	var beforeSqrt = GetInt().Set(x3)
	x.Exp(x3, u, sm2.P) //(x^3-3x+b)^2

	//check
	x3.Mul(x, x)
	x3.Mod(x3, sm2.P)
	if beforeSqrt.Cmp(x3) != 0 {
		PutInt(u)
		PutInt(beforeSqrt)
		PutInt(x3)
