...
//tildeY is Y rightmost bit
func complementCoordinates_64bit(x *big.Int, tildeY byte) error {
	var u big.Int
	var x1, x3 [4]uint64
	fromBig(&x1, x)
	p256Mul(&x1, &x1, &RR)
	p256Sqr(&x3, &x1, 1)
	p256Mul(&x3, &x3, &x1) //x^3

	p256Sub(&x3, &x3, &x1)
	p256Sub(&x3, &x3, &x1)
	p256Sub(&x3, &x3, &x1) //x^3-3x

	p256Add(&x3, &x3, &curveB) //x^3-3x+b

	//part1 appendix B.1.4, p = 3 mod 4
	//4u+3=p, u=3fffffffbfffffffffffffffffffffffffffffffc00000003fffffffffffffff
	fromMont(&x3, &x3)
	x.SetBits([]big.Word{big.Word(x3[0]), big.Word(x3[1]), big.Word(x3[2]), big.Word(x3[3])})
	u.SetBits([]big.Word{0x4000000000000000, 0xffffffffc0000000, 0xffffffffffffffff, 0x3fffffffbfffffff})
	x.Exp(x, &u, sm2.P)
	...
	fromMont(&x1, &x1)
	if x1 != x3 {
		return fmt.Errorf("it's not a quadratic residue")
	}

	if byte(x.Bit(0))+2 == tildeY {
		return nil
	}
	x.Sub(sm2.P, x)
	return nil
